...
import (
	...
	logger "github.com/YarBor/BorsMqServer/logger"
...
type MetaDataController struct {
	...
	wg           sync.WaitGroup
	MetaDataRaft *raft_server.RaftNode
	mu           sync.RWMutex
	MD           *MetaData
}
...
func (mdc *MetaDataController) SetRaftNode(rf *raft_server.RaftNode) {
	mdc.mu.Lock()
	defer mdc.mu.Unlock()
	mdc.MetaDataRaft = rf
}
...
func (mdc *MetaDataController) ConfirmIdentity(target *api.Credentials) error {
	if !mdc.IsLeader() {
		return err_.ErrRequestNotLeader
	}
	mdc.mu.RLock()
	defer mdc.mu.RUnlock()
...
func (mdc *MetaDataController) GetTopicTermDiff(Src map[string]int32) (map[string]int32, error) {
	if !mdc.IsLeader() {
		return nil, err_.ErrRequestNotLeader
	}
	mdc.mu.RLock()
	defer mdc.mu.RUnlock()
...
func (md *MetaData) createConsGroupTerm(ConsGroupID string) (int32, error) {
	...
	res, ok := md.ConsGroupTerm[ConsGroupID]
	if ok {
		return atomic.LoadInt32(res), err_.ErrSourceAlreadyExist
	} else {
...
func (md *MetaData) createTpTerm(TpName string) (int32, error) {
	...
	res, ok := md.TpTerm[TpName]
	if ok {
		return *res, err_.ErrSourceAlreadyExist
	} else {
...
func (md *BrokerMD) Copy() *BrokerMD {
	tb := *md
	return &tb
}
...
func ErrToResponse(err error) *api.Response {
	...
	switch {
	case errors.Is(err, err_.ErrFailure):
		return ResponseFailure()
	case errors.Is(err, err_.ErrSourceNotExist):
		return ResponseErrSourceNotExist()
	case errors.Is(err, err_.ErrSourceAlreadyExist):
		return ResponseErrSourceAlreadyExist()
	case errors.Is(err, err_.ErrSourceNotEnough):
		return ResponseErrSourceNotEnough()
	case errors.Is(err, err_.ErrRequestIllegal):
		return ResponseErrRequestIllegal()
	case errors.Is(err, err_.ErrRequestTimeout), err.Error() == "commit timeout":
		return ResponseErrTimeout()
	case errors.Is(err, err_.ErrRequestServerNotServe):
		return ResponseNotServer()
	case errors.Is(err, err_.ErrRequestNotLeader), err.Error() == "not leader":
		return ResponseErrNotLeader()
	case errors.Is(err, err_.ErrNeedToWait):
...
func (md *MetaData) GetFreeBrokers(brokersNum int32) ([]*BrokerData, error) {
	...
	if brokersNum > int32(len(md.Brokers)) {
		return nil, err_.ErrSourceNotEnough
	}
	...
	for i := range tmp {
		...
		logger.DEBUG(fmt.Sprintf("Add %s PartitionNum to %d", bks[i%len(bks)].ID, bks[i%len(bks)].PartitionNum))
...
func (mdc *MetaDataController) Handle(command interface{}) (err error, retData interface{}) {
	...
	defer func(err *error, retData *interface{}) {
		if mdCommand.Mode != KeepAlive {
			logger.DEBUG("recv ", mdCommand, "return ", *err, *retData)
		}
	}(&err, &retData)
	...
	case KeepAlive:
		...
		mdc.MD.bkMu.RUnlock()
		if !okFind {
			return err_.ErrSourceNotExist, nil
		} else {
			bk.ResetTimeoutTime()
			return nil, nil
		}
	...
	case
		...
		mdc.MD.pMu.Lock()
		if errCheck := mdc.MD.CheckProducer(id); errCheck == nil {
			panic("Ub")
		} else {
	...
	case
		UnRegisterConsumer:
		ConId, okTrans := mdCommand.Data.(*string)
		if !okTrans {
			str := mdCommand.Data.(string)
			ConId = &str
		}
		mdc.MD.cMu.Lock()
		...
		targetGroup.mu.Lock()
		if _, Ok := targetGroup.ConsumersFcPart[targetConsumer.SelfId]; !Ok {
	...
	case
		UnRegisterProducer:
		ProID, okTrans := mdCommand.Data.(*string)
		if !okTrans {
			strProID := mdCommand.Data.(string)
			ProID = &strProID
		}
		mdc.MD.pMu.Lock()
	...
	case
		...
		cons, ok = mdc.MD.Consumers[data.SelfID]
		if !ok {
			err = err_.ErrSourceNotExist
			goto JcgFinish
		}
	...
	Failure:
		...
		group.mu.Lock()
		if _, ok := group.FocalTopics[data.Topic]; ok {
			delete(group.FocalTopics, data.Topic)
			err = mdc.MD.reBalance(group)
		} else {
			err = err_.ErrSourceNotExist
		}
	...
	case
		...
		if !okTrans {
			...
			for _, tm := range tBk {
				if tm == nil {
					data.Brokers = append(data.Brokers, nil)
					continue
				}
		...
		p = &PartitionMD{
			Part: data.Part,
			BrokerGroup: BrokersGroupMD{
				Members: make([]*BrokerData, 0, len(data.Brokers)),
			},
		}
		mdc.MD.bkMu.RLock()
		for _, brokerData := range data.Brokers {
			if brokerData != nil {
				atomic.AddUint32(&mdc.MD.Brokers[brokerData.ID].PartitionNum, 1)
			} else {
				brokerData1, err1 := mdc.MD.GetFreeBrokers(1)
				if err1 != nil {
					err = err1
					return
				}
				brokerData = brokerData1[0]
			}
			p.BrokerGroup.Members = append(p.BrokerGroup.Members, brokerData)
		}
		mdc.MD.bkMu.RUnlock()

		t.Part.Parts = append(t.Part.Parts, p)
	...
	AddFalse:
		...
		for _, i := range needReBalance {
			...
			if ok {
				groupMD.mu.Lock()
				c, ok := groupMD.FocalTopics[data.Topic]
				if ok {
...
reHash:
	...
	if err != nil {
		if errors.Is(err, err_.ErrSourceAlreadyExist) && !IsAssign {
			goto reHash
		}
...
func (md *MetaData) reBalance(g *ConsumersGroupMD) error {
	defer func(gr *ConsumersGroupMD) {
		logger.INFO("Do ReBalance", g, gr)
	}(g)
	...
	for T, val := range g.FocalTopics {
		...
		if i, err := md.getTpTerm(T); err != nil {
			...
			if err == nil {
				...
				val.Parts = append(make([]*PartitionMD, 0, len(tp.Part.Parts)), tp.Part.Parts...)
		...
		if req.ConsumerData != nil && req.ConsumerData.ConsumerId != nil {
			...
			if req.ConsumerData.GroupTerm == tm {
				...
				for consID, fcps := range group.ConsumersFcPart {
					...
					if err != nil {
						logger.ERROR("Error querying consumer")
						continue
					}
...
func (md *MetaData) BrokersSourceCheck() error {
	...
	for _, brokerMD := range md.Brokers {
		if atomic.LoadInt32(&brokerMD.IsConnect) == BrokerMode_BrokerDisconnected {
			count++
		}
	}
	if count > len(md.Brokers) {
		return (err_.ErrFailure)
	}
	return nil
}
...
func (mdc *MetaDataController) ConsumerDisConnect(info *api.DisConnectInfo) *api.Response {
	...
	for _, part := range data.ConsumersData.FcParts {
		if part.ConsumerID == nil {
			logger.FATAL("Consumer Protocol Error")
		}
